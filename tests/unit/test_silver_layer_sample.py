...
class TestValidateDatetimeColumns:
    ...
    def test_valid_timestamps(self, spark):
        ...
        assert "tpep_pickup_datetime" in result_columns
        assert "tpep_dropoff_datetime" in result_columns
        assert result.filter(F.col("tpep_pickup_datetime").isNotNull()).count() == 1
    ...
    def test_null_timestamps(self, spark):
        ...
        assert result.filter(F.col("tpep_pickup_datetime").isNull()).count() == 1
        assert result.filter(F.col("tpep_dropoff_datetime").isNull()).count() == 1
